    ...
    for (const job of result.jobs) {
      try {
        ...
        const locationId = job.city && job.countryCode
          ...
          : null;
        const { roleId, occupationId } = inferRoleAndOccupation(job.title, roles);

        const result = await pool.query(
          `
          insert into jobs (
            external_id, ats_source_id, company_id, role_id, occupation_id,
          ...
          `,
          [
            job.externalId, atsSourceId, companyId, roleId, occupationId,
...
async function loadRoles(pool: Pool): Promise<RoleEntry[]> {
  const res = await pool.query<{
    ...
    `select r.id, r.name, r.synonyms, o.id as occupation_id
     from roles r
     left join occupations o on o.slug = r.occupation_slug`,
  );
...
function inferRoleAndOccupation(
  title: string,
  roles: RoleEntry[],
): { roleId: number | null; occupationId: number | null } {
  const t = title.toLowerCase();
  let best: RoleEntry | null = null;
  let bestLen = 0;
  ...
  return {
    roleId: best?.id ?? null,
    occupationId: best?.occupationId ?? null,
  };
}
